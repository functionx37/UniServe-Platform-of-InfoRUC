...
public class AdminService {
    ...
    public ImportPartyProgressResult importPartyProgress(String fileName, List<ImportPartyProgressRow> rows, Long operatorId) {
        ...
        try {
            ...
            for (PartyStage s : stages) {
                stageMap.put(s.getTitle(), s.getId());
            }
            for (int i = 0; i < rows.size(); i++) {
                ...
                try {
                    ...
                    String studentNo = row.getStudentNo().trim();
                    User user = userMapper.selectOne(new LambdaQueryWrapper<User>().eq(User::getStudentNo, studentNo));
                    if (user == null) {
                        throw new RuntimeException("未能找到该学号对应的用户: " + studentNo);
                    }
                    Integer stageId = stageMap.get(row.getStageTitle().trim());
                    if (stageId == null) {
                        throw new RuntimeException("无效的党团阶段名称: " + row.getStageTitle());
                    }
                    
                    UserPartyProgress progress = userPartyProgressMapper.selectOne(
                        new LambdaQueryWrapper<UserPartyProgress>()
                            .eq(UserPartyProgress::getUserId, user.getId())
                            .eq(UserPartyProgress::getStageId, stageId)
                    );
                    ...
                    if (progress == null) {
                        ...
                        progress.setCompleted(row.getCompleted() != null ? row.getCompleted() : true);
                        if (Boolean.TRUE.equals(progress.getCompleted())) {
                            progress.setCompletedAt(LocalDateTime.now());
                        }
                        progress.setNotes(row.getNotes());
