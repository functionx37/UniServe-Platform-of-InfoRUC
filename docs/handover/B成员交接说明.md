...
- 已适配仓库样例 `file/成绩单示例/示例成绩单.pdf`
- 对样例这类“按学期展示课程名、去掉成绩和逐门学分”的 PDF 做了专门解析
...
- 若未上传培养方案，则默认使用仓库样例 `file/培养方案示例/培养方案示例.xlsx`
- 服务端把样例或已上传 Excel 解析成结构化规则，并生成 `*.processed.json`
- 当前支持 `必修 / 任选一门 / 五选二 / 至少选修一门`
- 学生端分析时按课程匹配 + 规则组完成度做比对
- 输出概览、模块进度、未完成必修、未满足规则组、当前学期选课建议

当前限制：

- 上传的培养方案必须与样例文件保持相同表头和列顺序
- 若格式不一致，`POST /admin/curriculum/upload` 会直接返回“培养方案格式错误”
...
### 4.3 培养方案样例当前主要是“规则完成度”分析

当前仓库样例 `file/培养方案示例/培养方案示例.xlsx` 只有：
...
- 能识别必修/任选/至少选若干门
- 能根据当前学期输出建议
- 但 `metricLabel` 很可能是“课程要求单元”，不是严格学分

后续建议：

- 培养方案改成显式配置“课程学分 + 模块最低要求”
- 或者补充一张标准化 JSON/Excel 模板

### 4.4 PDF 成绩单解析仍依赖文本可提取

当前仅适配：

- 文本型 PDF
- 样例这种可提取文本、课程名能识别的成绩单
...
3. 至少再用一份真实成绩单 PDF 和一份真实培养方案 Excel 跑解析
